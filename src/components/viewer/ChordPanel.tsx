...
import { derivedShape } from "../../lib/voicings";
    ...
    <div className="rounded-[24px] bg-white p-4 text-center shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
      ...
      {nextName ? (
        <>
          <ChordName
            name={nextName}
            className="mb-2 block text-3xl font-bold tracking-tight text-slate-900"
          />
      ...
      {afterName && (
        <div className="mt-4 border-t border-slate-100 pt-3 opacity-60">
          ...
          <ChordName name={afterName} className="mb-1 block text-xl font-bold text-slate-700" />
...
export function ChordPanel({ names, activeName, jazz, onJazzChange }: Props) {
  ...
  const missing = names.filter((n) => !findChordShape(n) && !derivedShape(n));
    ...
    <div className="rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
      ...
      <div className="grid max-h-[420px] grid-cols-2 gap-3 overflow-y-auto pl-1">
        {names.map((name) => (
          ...
          >
            <ChordName name={name} className="mb-1 block text-center text-xs font-bold text-slate-900" />
      ...
      {missing.length > 0 && (
        <p className="mt-3 text-[11px] leading-relaxed text-slate-400">
          עדיין אין דיאגרמה ל־<span dir="ltr">{missing.map(prettyChord).join(", ")}</span> —
          המילון בהשלמה.
        </p>
      )}
    </div>
...
function Voicing({
  ...
  if (!entry) {
    // Not in the dictionary — a transposed sheet reaches chord types written
    // down at one root only, so fall back to sliding that shape here.
    const derived = derivedShape(name);
    if (derived) return <ChordDiagram shape={derived} width={width} className="mx-auto block" />;
    return (
      <div
